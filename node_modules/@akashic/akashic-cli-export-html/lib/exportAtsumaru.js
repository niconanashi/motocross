...
var bundle_1 = require("@akashic/akashic-cli-export-zip/lib/bundle");
var exportHTML_1 = require("./exportHTML");
var apiUtil_1 = require("./apiUtil");
function promiseExportAtsumaru(param) {
    if (param.output === undefined) {
        throw new Error("--output option must be specified.");
    }
    var outZip = path.extname(param.output) === ".zip";
    var destDir = outZip ? undefined : param.output;
    var completedParam = exportHTML_1._completeExportHTMLParameterObject(__assign({}, param));
    return exportHTML_1.promiseExportHTML(__assign({}, param, { output: destDir, logger: completedParam.logger }))
        .then(function (dest) {
        ...
        return bundle_1.promiseExportZip({
        ...
        // game.jsonへの追記
        var gameJson = require(path.join(completedParam.output, "game.json"));
        ...
        if (!gameJson.environment.niconico || !gameJson.environment.niconico.supportedModes) {
            ...
            completedParam.logger.warn("'environment.niconico.supportedModes', a required property for '--atsumaru' mode," +
        ...
        return apiUtil_1.getFromHttps("https://raw.githubusercontent.com/akashic-games/akashic-runtime-version-table/master/versions.json").then(function (data) {
        ...
        return new Promise(function (resolve, reject) {
            var ostream = fs.createWriteStream(path.resolve(param.cwd, param.output));
...
exports.promiseExportAtsumaru = promiseExportAtsumaru;
